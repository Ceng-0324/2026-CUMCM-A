...
def run(output_dir):
    ...
    summary = dict(scope='Q1 physical assumption audit; scenarios, not identified replacement answers',
                   ...
                   source_sha256={p:sha256((ROOT/p).read_bytes()).hexdigest() for p in
                       ['code/q1/q1_physics_audit.py','code/common/data_io.py','results/q1/fields.npz']},
    ...
    write_json(output_dir/'manifest.json',{p.name:sha256(p.read_bytes()).hexdigest()
        for p in [output_dir/'summary.json',output_dir/'histories.npz']})
