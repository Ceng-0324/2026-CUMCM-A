...
% 显式指定当前 macOS 可访问的中文字体，避免 fontset=mac 对 STHeiti 别名解析失败。
...
\newCJKfontfamily\songti[Path=/System/Library/Fonts/Supplemental/,Extension=.ttc]{Songti}
\newCJKfontfamily\heiti[Path=/System/Library/Fonts/,Extension=.ttc]{STHeiti Medium}
\newCJKfontfamily\kaishu[Path=/System/Library/AssetsV2/com_apple_MobileAsset_Font8/88d6cc32a907955efa1d014207889413890573be.asset/AssetData/,Extension=.ttc]{Kaiti}
% 西文正文与代码等宽字体：
...
% 封面（不编号）
\papertitle{药材的烘干问题：径向热质传递、达标事件与收缩机制}

% 摘要页（不编号）
\abstractcn{%
  本文针对药材热风烘干过程中的温度场、水分场、全域达标时间及收缩效应建立统一的径向传递模型。问题一采用常热物性与非线性水分扩散模型，问题二引入含水率相关热物性和温度相关扩散率，问题三以连续重构场的最大含水率定位首次达标时刻，问题四在材料坐标下处理给定收缩轨迹并分解几何与物性作用。计算采用环形有限体积、Kirchhoff 水分通量、Robin 边界半控制体和自适应 BDF。固定半径附录 3 条件下的连续达标时刻为 57.4741 h，收缩与附录 4 条件下为 51.0922 h。网格、时间容差、环境延拓及半径插值对照表明，结果在当前条件模型下具有数值稳定性；四组合分析显示收缩几何与整组物性变化存在显著竞争及交互作用。%
}{%
  药材干燥 \quad 热质耦合 \quad 有限体积法 \quad 收缩域 \quad 达标事件%
}
