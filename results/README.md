...
`probes/` 仍是可行性原型，事件取最大单元浓度，尚未认证连续域最大值或正式四位小数精度。Q1 已完成条件径向模型下的数值验证，详见 [结果报告](../reports/q1/RESULTS_REPORT.md)；Q2 已完成前 3 h 条件模型结果；Q3–Q4 正式结果尚未生成。输入目录中的同名文件是原始模板。模型口径与待办见 [总文档](../reports/ANALYSIS_MODELING_REPORT.md)。
